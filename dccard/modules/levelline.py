...
class Levelline(main_modules):
    def __init__(self,image,draw,nowlevel,nextlevel,linestyle=None,**args) -> None:
    ...
    def render(self,data):
        ...
        if self.linestype == 'rectangle':
            ...
            # 等級底色
            self.draw.rectangle((mainpos[0],mainpos[1],mainpos[2],mainpos[3]),fill=(0,0,0,0),round=10)
            # 等級條red color
            if level <= mainpos[2]:
                self.draw.rectangle((mainpos[0],mainpos[1],level+mainpos[0],mainpos[3]),fill=(255,0,0,255),round=10)
            else:
                self.draw.rectangle((mainpos[0],mainpos[1],level,mainpos[3]),fill=(255,0,0,255),round=10)